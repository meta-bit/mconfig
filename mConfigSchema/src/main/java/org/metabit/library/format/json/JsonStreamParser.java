...
public class JsonStreamParser
{
     ...
     */
    public static void parseJson(String input, JsonStreamConsumer consumer) throws JsonParsingException
        {
        new JsonStreamParser().parse(input, consumer);
        }

    public static void parseJson(Reader input, JsonStreamConsumer consumer) throws JsonParsingException
        {
        new JsonStreamParser().parse(input, consumer);
        }

    public static void parseJson(java.io.InputStream input, java.nio.charset.Charset charset, JsonStreamConsumer consumer) throws JsonParsingException
        {
        new JsonStreamParser().parse(new java.io.InputStreamReader(input, charset), consumer);
        }

    /**
     * Parsing convenience function, wrapping InputStream inputs.
     * Defaults to UTF-8 charset as per RFC 8259.
     *
     * @param input    InputStream as input
     ...
     */
    public static void parseJson(java.io.InputStream input, JsonStreamConsumer consumer) throws JsonParsingException
        {
        parseJson(input, java.nio.charset.StandardCharsets.UTF_8, consumer);
        }
    ...
    private void parseValue()
        ...
        char c = getNextCharSkippingWhitespace(); // might end
        parseValueStarted(c);
        }
    ...
    private void parseArrayStarted()
        ...
        do
            {
            char c = getNextCharSkippingWhitespace();
            if (c == ']')
                {
                loop = false;
                continue;
            ...
            if (!first) // separating ,
                ...
                c = getNextCharSkippingWhitespace();
                }
            else
                { first = false; }
    ...
    private void parseNumberStarted(char c)
        ...
        if (c == '0')
            ...
            if (next == '.' || next == 'e' || next == 'E')
                ...
                sb.append('0');
                c = next;
                // continue with regular number parsing
                }
            else
                {
                output.consumeNumberInteger(line, column, level, 0);
                putback = true;
                return;
                }
            }
        ...
        try // conversion, depending on flags and sizes
            ...
            if (hasExponent)
                ...
                {
                /*
                 * We switch from double to BigDecimal if the number of digits exceeds 15.
                 * A Java double (IEEE 754) provides approximately 15-17 significant decimal digits.
    ...
    private String readStringStarted()
        ...
        do
            {
            c = readNext(); // throw exception if stream ends during reading.
            switch (c)
                {
                case '\\':
                    {
     ...
     */
    private char getNextCharSkippingWhitespace()
        {
        char c;
        do
            {
    ...
    private char readNext()
        ...
        try
            ...
            if (currentChar < 0)
                ...
                if (level > 0)
                    {
                    throw new JsonParsingException(line, column, "unexpected end of input (level " + level + ")");
                    }
                else
                    {
                    return Character.MIN_VALUE; // use NUL as in-band signal for EOF at top level.
                    }
                }
            c = (char) currentChar;
            }
