...
public class SelfConfigurationComponent implements ConfigFactoryComponent
{
    ...
    private void applyProperties(ConfigFactorySettings settings, Properties props, ConfigLoggingInterface logger)
        ...
        for (String key : props.stringPropertyNames())
            ...
            try
                ...
                org.metabit.platform.support.config.ConfigFeatureInterface feature = org.metabit.platform.support.config.ConfigFeatureRegistry.get(key);
                if (feature != null)
                    {
                    applyValue(settings, feature, value);
                    logger.debug("Applied self-configuration: "+key+" = "+value);
                    }
                else
                    {
                    settings.setFeature(key, value);
                    logger.debug("Applied custom self-configuration: "+key+" = "+value);
                    }
                }
            catch (Exception e)
                {
    ...
    private void applyValue(ConfigFactorySettings settings, org.metabit.platform.support.config.ConfigFeatureInterface feature, String value)
        {
