...
public class InMemoryLayer implements ConfigLayerInterface
{
    ...
    public void writeEntry(ConfigEntry entryToWrite)
        ...
        synchronized(lock)
            ...
            // Automatically trigger change check in the storage when writing
            if (this.source instanceof ConfigLocation)
                {
                ConfigLocation location = (ConfigLocation) this.source;
                location.getStorage().triggerChangeCheck(location.getStorageInstanceHandle());
                }
            }
        }
